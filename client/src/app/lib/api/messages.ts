...
export const messagesApi = {
    ...
    getMessageHistory: async (channelId: string): Promise<Message[]> => {
        ...
        return [...(mockMessages[numericChannelId] || [])];
    },

    // POST /channels/{id}/messages 
    send: async (channelId: string, content: string, user?: User): Promise<Message> => {
        if (!user) {
        ...
        if (!mockMessages[numericChannelId]) {
            mockMessages[numericChannelId] = [];
        }

        mockMessages[numericChannelId].push(newMessage);
    ...
    delete: async (channelId: string, messageId: string): Promise<void> => {
        const numericChannelId = Number(channelId);
        if (mockMessages[numericChannelId]) {
            mockMessages[numericChannelId] = mockMessages[numericChannelId].filter(m => m.id !== messageId);
        }
    }
};